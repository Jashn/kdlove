...
function __construct() {
    ...
    public function vendor_category(){
      ...
      $config = array();
      $config["base_url"] = base_url() . "home/vendor_category/";
     ...
     $data['vendor_category'] =  $this->Frontend_model->getAllValues('vendor_category','');

     $data['vendor_item'] = $this->Frontend_model->fetch_data($page,'vendor_cat_id','wedding_marketplace_items',$config["per_page"]);
     
     $str_links = $this->pagination->create_links();
     $data["links"] = explode('&nbsp;',$str_links );
     
    //$this->site_data['vendor_item'] = $this->Frontend_model->getAllValueByid($id,'vendor_cat_id','wedding_marketplace_items');
      //$this->site_data['main_page'] = 'vendor_item_view';
      $this->load->view('vendor_item_view', $data);
   }
   ...
   public function dashboard(){
    ...
    if(!$this->session->userdata('email')){
     ...
     $profile_details =  $this->Frontend_model->get_loginUser($email,$password);
   ...
   public function user_checklist(){
      ...
      $email =  $this->session->userdata('email');
      $password =  $this->session->userdata('password');
      $user_info =  $this->Frontend_model->get_loginUser($email,$password);
      $checklist_to_do = $this->Frontend_model->get_checklist('1');
      $get_cheked_to_do =  $this->Frontend_model->get_checklist_done('1');

      $checklist_budget = $this->Frontend_model->get_checklist('2');
      $checklist_my_vendor = $this->Frontend_model->get_checklist('3');

      $data = array(
        'user_info'=>$user_info,
        'checklist_to_do'=>$checklist_to_do,
        'get_cheked_to_do'=>$get_cheked_to_do,
        'checklist_budget'=>$checklist_budget,
        'checklist_my_vendor'=>$checklist_my_vendor,
        );
      $this->load->view('home/user_checklist',$data);
   }
   ...
   public function edit_profile(){
     ...
     $theme = $this->input->post('wedding_theme');
      ...
      $data['wedding_theme'] = $this->Frontend_model->getAllValues('wedding_theme','id');



     if(isset($email) && isset($password)){
      $data['user_info'] =  $this->Frontend_model->get_loginUser($email,$password);

       $id = $data['user_info']['id'];
      $data['wedding_details'] = $this->Frontend_model->get_valueByid($id,'user_id','wedding_details');
    ...
    // $this->site_data['main_page'] = 'home/user_profile';
...
*/
  ...
  if ( in_array($extension, $allowedExts))
    ...
    else
    {
      ...
      $data['profile_details'] =  $this->Frontend_model->get_loginUser($email,$password);
      $user_id = $data['profile_details']['id'];
      $update_image = array(
        'image'=>$response['url']
        );
