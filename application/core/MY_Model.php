...
class MY_Model extends CI_Model
{
    ...
    public function __construct()
    {
        ...
        $this->pagination_arrows = array('<span aria-hidden="true"><i class="fa fa-angle-left"></i></span>','<span aria-hidden="true"><i class="fa fa-angle-right"></i></span>');
...
//        $this->pagination_arrows = (isset($this->pagination_arrows)) ? $this->pagination_arrows : array('&lt;','&gt;');
    ...
    public function paginate($rows_per_page, $total_rows = NULL, $page_number = 1)
    {
     ...
     public function product_paginate_customized($rows_per_page, $total_rows = NULL, $page_number = 1,$filter_page)
    {
        $this->load->helper('url');
        $segments = '';
        $uri_array = array();
        ...
        if (!empty($filter_page)) {
            $page_number = $filter_page;
        } else if (empty($filter_page)) {
            $segments = $this->uri->total_segments();
            $uri_array = $this->uri->segment_array();
            $page = $this->uri->segment($segments);
            if (is_numeric($page)) {
        ...
        if(isset($total_rows))
        {
            if($total_rows!=0)
            {
                ...
                for ($i = 1; $i <= $number_of_pages; $i++) {
                    ...
                    $uri_string = implode('/', $uri_array);
                    $links .= $this->pagination_delimiters[0];
                    $links .= (($page_number == $i) ? anchor($uri_string, $i) : anchor('admin/product' . '/' . $i, $i));
                    $links .= $this->pagination_delimiters[1];
                }
                $links .= $this->next_page;
                $this->all_pages = $links;
